...
- `references/tourist-mode-notes.md` — 游客模式实测行为记录（哪些功能可用/有问题/配置正式账号）
...
|------|---------|------|
| `search_symbols` | ✓ | 正常 |
| `market_data_cn`（个股行情） | ✓ | 正常 |
| `call_auction_anomaly`（竞价异动） | ✓ | 返回1000条，但金额字段疑似脱敏（值=2147483648占位） |
| `news`（实时快讯） | ✓ | 正常，20条 |
| `market_data_index`（指数行情） | ✗ | 返回空数据 |

**结论**：游客模式可用于快速尝鲜，但数据完整性和稳定性无法保证。竞价异动的金额数据不可信；指数行情（上证/深证等）无数据。日常使用建议配置正式账号。
...
正式账号获得：稳定连接、完整数据（无脱敏占位）、更高的频次配额。
