#!/usr/bin/env python3
"""
每日财经资讯 v2（基于 v1 调通版重构）

数据源：
  1. 大盘指数（7个）：A股+港股+纳指 → 腾讯行情；A股昨日 → EM历史K线
  2. 关注板块ETF → 腾讯行情
  3. 基金净值 → EM历史NAV + 1234567今日估算
  4. A股持仓 → thsdk；港股持仓 → 腾讯行情（thsdk无数据）
  5. 异动资讯 → thsdk问财 + 快讯
"""
import subprocess, re, json, time, os, urllib.request, thsdk as _thsdk
...
def _call_thsdk(func, *args, seconds=10, **kwargs):
    ...
    if t.is_alive():
        print(f"  [WARN] thsdk 超时 ({seconds}s)")
        return None
...
def lark_cli(cmd_list, data_str=None, cwd=WORKSPACE):
    ...
    if proc.returncode == 0:
        ...
        except: return {"ok": False, "error": proc.stdout}
    return {"ok": False, "error": proc.stderr or proc.stdout}


# ========== 腾讯行情 API ==========
def _qq_quote(codes):
    """批量获取腾讯行情，返回 {code: {name, price, preclose, pct, open}}"""
    ...
    for line in raw.strip().split("\n"):
        parts = line.split("~")
        if len(parts) > 35:
            inner = parts[0].split("=")[0]
            code = inner.lstrip("v_")
...
# ========== 1. 大盘指数（腾讯行情 + EM历史K线）==========
# EM secid 映射：A 股指数昨日从 EM 历史 K 线获取
EM_ASTOCK_MAP = {
...
def _load_market_cache():
    try:
        ...
        files = [f for f in os.listdir(MARKET_CACHE_DIR)
                 if f.startswith("market_cache_") and f.endswith(".json")]
        if not files: return {}
        with open(os.path.join(MARKET_CACHE_DIR, sorted(files)[-1]), encoding="utf-8") as f:
            cache = json.load(f)
        print(f"  [缓存] 读取 {files[-1]}")
        return cache
    except Exception:
        return {}
...
def _save_market_cache(indices):
    try:
        ...
        for idx in indices:
            code = idx.get("代码", "")
            cache[code] = {"昨日价": idx.get("最新价", "-"), "昨日涨跌幅": idx.get("最新涨跌幅", "-")}
        cache["_date"] = today
        path = os.path.join(MARKET_CACHE_DIR, f"market_cache_{today}.json")
...
def _get_em_yesterday(secid):
    """查东方财富历史K线，返回 (date, close, yest_pct) 或 None"""
    try:
        today = datetime.now().strftime("%Y%m%d")
...
def get_market_indices():
    """获取7个大盘指数：A股+港股+纳指 → 腾讯行情；A股昨日 → EM历史K线"""
    ...
    for qq_code, name, em_secid in index_configs:
        ...
        try:
            ...
            today_pct = float(d.get("pct",     "0"))
        except (ValueError, TypeError):
            results.append({"指数名称": name, "代码": qq_code,
                             "昨日价": "-", "昨日涨跌幅": "-", "最新价": "-", "最新涨跌幅": "-"})
            continue

        # 昨日涨跌幅
        yest_close, yest_pct = preclose, "-"
        if em_secid:  # A股：从 EM 历史 K 线获取
            em = _get_em_yesterday(em_secid)
            if em:
                yest_close = round(float(em[1]), 2)
                yest_pct = em[2]
        if isinstance(yest_pct, str) and qq_code in cache:
            # 港股：直接从缓存读
    ...
    try:
        ...
        if price > 0:
            ...
            results.append({"指数名称": "纳斯达克", "代码": "IXIC",
                             "昨日价": "-", "昨日涨跌幅": "-", "最新价": "-", "最新涨跌幅": "-"})
    except (ValueError, TypeError):
        results.append({"指数名称": "纳斯达克", "代码": "IXIC",
                         "昨日价": "-", "昨日涨跌幅": "-", "最新价": "-", "最新涨跌幅": "-"})
...
def get_etf_list():
    """从本地 JSON 读取ETF列表（由 cron job 预填充）"""
    try:
        if os.path.exists(ETF_LIST_JSON):
            with open(ETF_LIST_JSON, encoding="utf-8") as f:
                etfs = json.load(f)
            if etfs:
                print(f"  [INFO] 从 etf_list.json 加载 {len(etfs)} 条ETF")
                return etfs
    except Exception as e:
...
def _code_to_qq(code):
    """ETF代码 → 腾讯行情代码"""
    code = str(code).strip()
    if code.startswith(("159", "150", "501")):
        return f"sz{code}"
    return f"sh{code}"
...
def get_sector_etfs():
    ...
    if not etf_list:
        etf_list = [
            ...
            {"sector": "智能电网", "name": "易方达恒生A股电网设备 ETF",  "code": "561380"},
    ...
    for etf in etf_list:
        d = qq_data.get(_code_to_qq(etf["code"]), {})
        try:
            price    = float(d.get("price",    "0"))
            preclose = float(d.get("preclose", "0"))
            ...
            results.append({"所属板块": etf["sector"], "ETF名称": etf["name"], "ETF代码": etf["code"],
                             "昨日收盘价": "-", "昨日涨跌幅": "-", "最新价": "-", "最新涨跌幅": "-"})

    results.sort(key=lambda x: (order.get(x["所属板块"], 99), x["ETF名称"]))
    return results


# ========== 3. 基金净值（EM历史 + 1234567今日估算）==========
def _get_last_nav(code):
    """EM历史NAV接口：返回 (date_str, dwjz, jzzzl)"""
    try:
        today  = datetime.now().strftime("%Y-%m-%d")
        start  = (datetime.now() - timedelta(days=10)).strftime("%Y-%m-%d")
        url    = (f"https://api.fund.eastmoney.com/f10/lsjz?fundCode={code}"
                  f"&pageIndex=1&pageSize=5&startDate={start}&endDate={today}")
...
def get_fund_navs():
    ...
    for code, name in funds:
        _, nav_val, nav_pct = _get_last_nav(code)
        gsz, gszzl = "", ""
        try:
            ...
            if m:
                ...
                gsz = d.get("gsz", ""); gszzl = d.get("gszzl", "")
        except:
            pass
        results.append({
            "基金代码":   code, "基金名称": name,
...
FEISHU_HOLDINGS_TBL = "tbl8hhmWssnxpmFg"
...
def get_holdings_from_bitable():
    ...
    while True:
        url = f"https://open.feishu.cn/open-apis/bitable/v1/apps/{FEISHU_APP_TOKEN}/tables/{FEISHU_HOLDINGS_TBL}/records?page_size=100"
    ...
    for rec in records:
        ...
        sc  = f.get("股票代码", "")
        pct = f.get("持仓比例")
        if fc and sn and pct:
            holdings.setdefault(fc, []).append((sn, str(sc).strip(), float(pct)))
...
def _to_ths_code(market, code):
    """转换为 thsdk THSCODE"""
    code = str(code).strip()
    if market == "SH": return f"USHA{code}"
    if market == "SZ": return f"USZA{code}"
    return None


# 持仓股代码映射（名称 → (市场, 代码)）
# 港股代码：德昌(00179)、中芯国际(00981)、阿里巴巴(09988)
# 注：bitable 存的是 00579（错误），v2 用 stock_code_map 修正为 00179
STOCK_CODE_MAP = {
    "新泉股份":   ("SH", "603179"),
    "斯菱智驱":   ("SZ", "301229"),
    "德昌电机控股":("HK", "00179"),   # bitable 存 00579，腾讯正确代码 00179
    "宁波华翔":   ("SH", "002048"),
    "五洲新春":   ("SH", "603667"),
    "航天电子":   ("SH", "600879"),
    "中国卫星":   ("SH", "600118"),
...
def get_holdings_table():
    ...
    for fc in fund_codes:
        if fc in holdings_json:
            for sn, sc, pct in holdings_json[fc]:
                holdings[fc].append((sn, sc, pct))
    ...
    for fc in fund_codes:
        for name, code_str, pct in holdings.get(fc, []):
            if name not in [s[0] for s in all_stocks] and name in STOCK_CODE_MAP:
                all_stocks.append((name, pct))

    stock_prices = {}

    # A股：thsdk 逐个查询（带超时）
    import threading as _threading
    for name, holding_pct in all_stocks:
        market, code = STOCK_CODE_MAP.get(name, (None, None))
        if market in ("SH", "SZ"):
            ths_code = _to_ths_code(market, code)
            if not ths_code: continue
            result = [None]
            def _job():
                try:
                    ...
                    result[0] = e
            t = _threading.Thread(target=_job); t.daemon = True; t.start()
            t.join(timeout=8)
            if t.is_alive():
                print(f"  [WARN] thsdk A股超时: {name}"); continue
            r = result[0]
            if isinstance(r, Exception):
                print(f"  [WARN] thsdk A股异常: {name} {r}"); continue
            if r and r.success and not r.df.empty:
                ...
                stock_prices[name] = {"preclose": preclose, "price": price,
                                      "open": open_p, "pct": pct, "holding_pct": holding_pct}
            time.sleep(0.15)

    # 港股：腾讯行情（thsdk 无数据，直接用腾讯）
    hk_stocks = [(n, p) for n, p in all_stocks if STOCK_CODE_MAP.get(n, (None,))[0] == "HK"]
    if hk_stocks:
        qq_str = ",".join(f"hk{STOCK_CODE_MAP[n][1]}" for n, _ in hk_stocks)
        qq_data = _qq_quote(qq_str)
        for name, holding_pct in hk_stocks:
            _, code = STOCK_CODE_MAP[name]
            d = qq_data.get(f"hk{code}", {})
            try:
                price    = float(d.get("price",    0))
                preclose = float(d.get("preclose", 0))
                open_p   = float(d.get("open",     0))
    ...
    for fc in fund_codes:
        ...
        for name, code_str, holding_pct in holdings.get(fc, []):
            if name in stock_prices:
                ...
                preclose = pd["preclose"]; price = pd["price"]
                open_p   = pd["open"]; today_pct = pd["pct"]
                ...
                table_rows.append({
                    ...
                    "今日涨跌幅":   f"{'+' if today_pct > 0 else ''}{today_pct:.2f}%",
                    "_today_pct_raw": today_pct if isinstance(today_pct, float) else 0,
                })
            else:
                table_rows.append({
                    "所属基金": fund_name, "股票名称": name, "股票代码": code_str or "-",
                    "昨日收盘价": "-", "昨日涨跌幅": "-", "今日价格": "-", "今日涨跌幅": "-",
                    "_today_pct_raw": 0,
                })
    return table_rows


# ========== 5. 异动股票资讯（thsdk问财 + 快讯 + Tavily）==========
_wencai_cache = {"items": None}

HERMES_ENV = "/Users/twliang/.hermes/.env"
TAVILY_SEARCH_SCRIPT = "/Users/twliang/.hermes/skills/research/tavily-search/scripts/search.mjs"
...
def _tavily_search_news(query, count=3):
    """用 Tavily 搜索新闻，返回 [(title, body, url), ...]"""
    key = _get_tavily_key()
    if not key:
        return []
    try:
        ...
        result = subprocess.run(
            ["node", TAVILY_SEARCH_SCRIPT, query, "-n", str(count), "--topic", "news"],
        ...
        for item in data.get("results", []):
            results.append((item.get("title", ""), item.get("content", "")[:200], item.get("url", "")))
        return results
    except Exception as e:
        return []
...
def get_stock_news(stock_code, stock_name, count=3):
    ...
    try:
        ...
        if anomalies:
            kw = stock_name
            code_digits = re.sub(r'\D', '', str(stock_code))
        ...
        # 3. Tavily 新闻搜索兜底
        if len(results) < count:
            tavily_results = _tavily_search_news(f"{stock_name} {stock_code} 股票 新闻", count - len(results))
            for title, body, url in tavily_results:
                results.append((title, body[:150], url, "Tavily"))
                if len(results) >= count:
                    break
    except Exception as e:
...
def main():
    ...
    print(f"[{datetime.now().strftime('%H:%M:%S')}] === 每日财经 v2 开始 ===")
    ...
    md_lines.append("|---------|---------|---------|---------|---------|---------|---------|")
    for row in table_rows:
        md_lines.append(f"| {row['所属基金']} | {row['股票名称']} | {row['股票代码']} | {row['昨日收盘价']} | {row['昨日涨跌幅']} | {row['今日价格']} | {row['今日涨跌幅']} |")
    md_lines.append("")
    ...
    if news_items:
        for code, (name, news_list) in news_items.items():
            ...
            for title, body, date, url in news_list:
                ...
                if body: md_lines.append(f"  {body.strip()}")
            md_lines.append("")
    else:
    ...
    # 写入临时文件
    md_path = "/Users/twliang/.hermes/tmp/daily_finance.md"
