...
When moving/renaming/migrating any skill directory, you MUST follow the safety checklist in:

→ `references/skill-migration-safety.md`

**核心风险**：skill 目录迁移后，cron job prompt 里的脚本路径**不会自动同步**，导致 cron 实际调用失败。这个问题已导致 system-check cron job 路径错误 3 次。迁移后必须执行完整检查清单。
