...
**Gateway installed as LaunchAgent does not stop with `kill PID`.**
When OpenClaw gateway is installed as a macOS LaunchAgent, sending `kill <PID>` alone is ineffective — launchd immediately restarts the process. Always use the proper service uninstall command first:

```bash
openclaw gateway uninstall   # removes LaunchAgent, then kills residual process
```

Do NOT use `openclaw service uninstall` — there is no `service` subcommand. The correct path is `openclaw gateway uninstall`.

After uninstall, verify with `ps aux | grep openclaw` to confirm no residual process remains.

**OpenClaw gateway and Hermes gateway run independently and can conflict.**
Both gateways can run simultaneously on different ports (OpenClaw default: 18789). If both have the same channel (e.g., Feishu) configured, they will conflict and cause channel instability. When troubleshooting Feishu or channel issues, check `ps aux | grep -E "(openclaw|hermes)"` to determine if both are running and consider stopping one if redundant.
