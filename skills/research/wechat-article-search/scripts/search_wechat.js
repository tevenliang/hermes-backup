...
async function httpGet(url, cookieStr = '') {
  ...
  const args = ['-s', '-L', '-A', UA, '--compressed'];
  const curlHeaders = {};
  if (cookieStr) curlHeaders['Cookie'] = cookieStr;
  Object.entries({ ...HEADERS, ...curlHeaders }).forEach(([k, v]) => {
    if (!['Host','Content-Length'].includes(k)) args.push('-H', `${k}: ${v}`);
  });
  args.push(url);
  const stdout = await new Promise((resolve, reject) => {
    execFile('curl', args, { timeout: 30000 }, (err, stdout, stderr) => {
...
function parseArticlesFromSearchHtml(html, maxResults) {
  ...
  // 新版搜狗页面结构：标题在 id='sogou_vr_11002601_title_N' 的 a 标签中
  // 摘要在 id='sogou_vr_11002601_summary_N' 的 p 标签中
  // 尝试新结构
  const titleCount = $('[id^="sogou_vr_11002601_title_"]').length;
  if (titleCount > 0) {
    $('[id^="sogou_vr_11002601_title_"]').each((_, el) => {
      ...
      // 找对应的摘要（相同序号）
      const idNum = (el.attribs && el.attribs.id) ? el.attribs.id.replace('sogou_vr_11002601_title_', '') : '';
      let summary = '';
      if (idNum !== undefined) {
        summary = $(`#sogou_vr_11002601_summary_${idNum}`).text().replace(/\s+/g, ' ').trim();
      }
      if (title && url) {
...
async function resolveRealUrls(articles) {
  ...
  for (let i = 0; i < articles.length; i++) {
    ...
    try {
      ...
      const isSuccess = !realUrl.includes('wx.sogou.com') && !realUrl.includes('antispider');
...
async function searchWechatArticles(query, maxResults = 10, resolveRealUrl = false) {
  ...
  while (articles.length < maxResults && page <= pagesNeeded) {
    try {
      ...
      const url = `https://wx.sogou.com/weixin?query=${encodedQuery}&s_from=input&_sug_=n&type=2&page=${page}&ie=utf8`;
