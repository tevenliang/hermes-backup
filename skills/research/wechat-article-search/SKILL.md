...
### 步骤1: 确认已安装依赖包
该脚本依赖 Node.js 依赖包 `cheerio`。先检查是否已安装：
```bash
npm list -g cheerio
```
若返回 empty，执行安装：
...
```
注意：依赖缺失时搜索脚本会静默失败（返回空 JSON），需确保安装成功。
...
**发布时间不直接在结构化结果中返回**。搜索结果包含标题、摘要、公众号名称、URL，但 publish_time 字段在搜索 API 响应中为空或不可解析。获取精确发布时间需要：
1. 打开每篇文章详情页（browser_navigate 或 opencli note）
2. 在页面中提取发布时间

**替代方案**：如果搜索目标是**小红书**平台，直接用 `opencli xiaohongshu search` 更优——返回结构化数据含标题、作者、点赞/收藏数、**精确发布日期**，无需二次抓取。微信公众号更适合长文章、深度内容。
