...
- **新闻资讯**: BBC, Bloomberg, Reuters, HackerNews, Dev.to, **36kr**
...
```
> **常见问题**：Chrome 扩展显示"未连接" → 运行 `opencli daemon stop && opencli doctor` 即可恢复连接。
...
```
> **重要**：`opencli` 命令默认输出 **YAML 格式**（不是 JSON）。在 Python 中解析需用 `yaml.safe_load()`，不要用 `json.loads()`，否则会报 `Expecting value: line 1 column 1`。
...
### 搜索
```bash
# 36kr 搜索（中文科技/商业新闻，公开 API，输出 YAML）
opencli 36kr search "比亚迪"
...
```bash
# 36kr
opencli 36kr news          # 最新科技新闻
opencli 36kr hot           # 热榜
opencli 36kr article <id>  # 获取文章正文（需从搜索结果取 id）
...
opencli hackernews search --keyword "OpenAI"

# Reuters
opencli reuters search "economy"
```
...
### 公开 API vs 浏览器命令
- **公开 API**: 不需要浏览器，直接访问（如 hackernews, bbc, 36kr, wikipedia）
...
### 扩展未连接（最常见问题）
Chrome 扩展显示"未连接"时，**不需要重新配对或手动操作**，修复步骤：
```bash
opencli daemon stop && opencli doctor
```
重启 daemon 后扩展会自动连接。如果仍有问题：
1. 确认扩展已在 `chrome://extensions/` 加载且启用
2. 点击 Chrome 工具栏的 OpenCLI 扩展图标，确保弹出窗口显示 connected
3. 运行 `opencli doctor --live` 测试实时浏览器命令

### 扩展安装步骤（完整流程）
1. 下载最新扩展：`curl -fsSL https://github.com/jackwener/opencli/releases/latest/download/opencli-extension-*.zip -o /tmp/opencli-extension.zip && unzip -o /tmp/opencli-extension.zip -d /tmp/opencli-extension`
2. Chrome 打开 `chrome://extensions/`
3. 右上角开启**开发者模式**
4. 点击"加载已解压的扩展程序" → 选择 `/tmp/opencli-extension`
5. 重启 daemon：`opencli daemon stop && opencli doctor`
...
1. **扩展未连接时**：优先 `opencli daemon stop && opencli doctor`，不需要重装扩展
2. **先检查登录状态**: 浏览器命令前确保已在 Chrome 登录
