...
# Setup: Configure Lark MCP Server in Hermes

This skill requires the official Lark MCP server running. Add to `~/.hermes/config.yaml`:
...
**Get credentials**: https://open.feishu.cn/app (China) or https://open.larksuite.com/app (International)

**Skill installation path**: Skills from Skillhub land in `~/.openclaw/workspace/skills/` — move to `~/.hermes/skills/` for Hermes to auto-load them.
