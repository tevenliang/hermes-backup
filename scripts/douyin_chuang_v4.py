#!/usr/bin/env python3
"""创哥的AI实验室 - v4 用 model=base 加快转写，加超时保护"""
...
for v in VIDEOS:
    ...
    print(f"\n[{VIDEOS.index(v)+1}/7] {title[:40]}... (时长={duration}s)")
    ...
    # 转写 - 用 model=base 提速，动态超时
    # model=base 比 small 快约3倍，适合 CPU
    # 超时保护：duration * 2 秒，但最少300秒，最多1200秒
    whisper_t = max(300, min(1200, duration * 2))
    print(f"  转写中 (model=base, timeout={whisper_t}s)...")
    run(f'whisper "{mp3}" --model base --language Chinese --output_dir "{TMP}" --output_format txt 2>/dev/null', timeout=whisper_t)
    ...
    if os.path.exists(txt) and os.path.getsize(txt) > 0:
        ...
        print(f"  转写文件不存在或为空（可能超时）")

    # 写入飞书
    print(f"  写入飞书...")
    result = write_feishu(aweme_id, title, content, f"https://www.douyin.com/video/{aweme_id}", duration, digg)
    print(f"  完成: {aweme_id}")
