...
# sec_uid | 博主名 | 是否已完成（部分完成）
# 数字游牧人Samuel 已全部完成，跳过
# 宋鸿兵观天下 完成3个，还剩6个
# 可喜 0个
# 创哥的AI实验室 0个
BLOGGERS = [
...
# 已完成的 aweme_id（从上次日志确认）
SKIPPED_IDS = {
    "MS4wLjABAAAAzaye_V0qtP4d7m77UywUBRq7xB9CRiLaeGPfg79hLtQ": [
        "7636711348637912370",
        "7634856987204816162",
        "7634514676063243558",
    ]
...
def process_blogger(sec_uid, blogger_name, existing_ids, extra_skip_ids=None):
    print(f"\n{'='*60}")
    print(f"处理博主: {blogger_name}")
    print(f"{'='*60}")

    skip_set = set(extra_skip_ids or [])
    ...
    for v in videos:
        ...
        if aweme_id in existing_ids or aweme_id in skip_set:
            print(f"  [SKIP] {aweme_id} 已存在/已处理")
            continue

        print(f"\n  [NEW] {title[:50]}... (时长={duration}s, 点赞={digg_count})")
        ...
        # Whisper转写 - 超时时间动态设置（短于5分钟的视频600s，长的给900s）
        whisper_timeout = 900 if duration > 300 else 600
        print(f"    转写中... (超时限制={whisper_timeout}s)...")
        run(f'whisper "{mp3}" --model small --language Chinese --output_dir "{TMP}" --output_format txt 2>/dev/null', timeout=whisper_timeout)
...
def main():
    ...
    for sec_uid, blogger_name in BLOGGERS:
        extra_skip = SKIPPED_IDS.get(sec_uid, [])
        total += process_blogger(sec_uid, blogger_name, existing_ids, extra_skip)
        # 更新已见ID
        existing_ids = get_existing_ids()
